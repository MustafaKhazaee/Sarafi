...
public interface IRepository<T> : IAsyncDisposable where T : AggregateRoot
{
    ...
    Task<List<T>> GetFilteredPageAsync(Expression<Func<T, bool>> predicate, int pageIndex = 0, int pageSize = 10, CancellationToken cancellationToken = default);
